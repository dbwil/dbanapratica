...
→ diretório utilizado para dados que variam durante o funcionamento do sistema.

lib

→ arquivos utilizados por aplicações/serviços.

pgsql

→ diretório relacionado ao PostgreSQL.
...
→ diretório principal de dados da instância, o PGDATA.

pg_hba.conf

→ arquivo de regras de autenticação.

Portanto:

/var/lib/pgsql/17/data/pg_hba.conf

é o arquivo de autenticação utilizado pela minha instância.


3. Saindo do PostgreSQL para lER o arquivo (/var/lib/pgsql/17/data/pg_hba.conf).

```bash
postgres=# \q
postgres@localhost ~]$ cat /var/lib/pgsql/17/data/pg_hba.conf
...
- **USER:** O campo especifica quais usuários do banco de dados (roles) aquela regra de autenticação se aplica.
- **ADDRESS:** De qual endereço/IP a conexão pode vir.
- **METHOD:** Define como a identidade do usuário será verificada quando ele tentar se conectar ao banco de dados.

Principais métodos (METHOD):
- **trust:** Permite a conexão incondicionalmente. O PostgreSQL confia na palavra do usuário e não pede senha.
- **reject:** Rejeita a conexão imediatamente. Útil para bloquear redes ou usuários específicos.
- **md5 / scram-sha-256:** Exige que o usuário forneça uma senha criptografada. O scram-sha-256 é o padrão mais moderno e seguro no PostgreSQL.
